...
(Aint.vs.Aex= full.exin.delta%>%
  ...
  ggscatter(x ="A.int", y="A.ex",color = "species",add = "reg.line", conf.int = TRUE) +
   theme_classic()+theme(axis.text.x=element_text(size=15),axis.text.y=element_text(size=15))+
    xlab(expression('Intact photosynthesis (μmol m'^-2*'s'^-1*')'))+ylab(expression('Excised photosynthesis (μmol m'^-2*'s'^-1*')'))+
...
(gint.vs.gex= full.exin.delta%>%
  ggscatter(x ="gs.int", y="gs.ex",color = "species",add = "reg.line", conf.int = TRUE) +
   theme_classic()+theme(axis.text.x=element_text(size=15),axis.text.y=element_text(size=15))+
    xlab(expression('Intact stomatal conductance (mol m'^-2*'s'^-1*')'))+ylab(expression('Excised stomatal conductance (mol m'^-2*'s'^-1*')'))+
...
(wpint.vs.wpex= full.exin.delta%>%
  ggscatter(x ="watpot.int.mpa", y="watpot.ex.mpa",color = "species",add = "reg.line", conf.int = TRUE) +
   theme_classic()+theme(axis.text.x=element_text(size=15),axis.text.y=element_text(size=15))+
    xlab(expression('Intact water potential (MPa)'))+ylab(expression('Excised water potential (MPa)'))+
...
(species.A=(ggplot(contrast_data.A, aes(x = species, y = estimate, fill = species, shape = species)) +
  ...
  theme_classic() +
  theme(axis.text.y = element_blank(),axis.text.x = element_text(size = 12),axis.title.x = element_text(size = 10),axis.title.y = element_text(size = 15),legend.title= element_blank()) +
...
(species.g=(ggplot(contrast_data.g, aes(x = species, y = estimate, fill = species, shape = species)) +
  ...
  theme_classic() +
  theme(axis.text.y = element_blank(),axis.text.x = element_text(size = 12),axis.title.x = element_text(size = 9),axis.title.y = element_text(size = 15),legend.title= element_blank()) +
...
(species.watpot=(ggplot(contrast_data.watpot, aes(x = species, y = estimate, fill = species, shape = species)) +
  ...
  theme_classic() +
  theme(axis.text.y = element_text(size = 12),axis.text.x = element_text(size = 12),axis.title.x = element_text(size = 10),axis.title.y = element_text(size = 15),legend.title= element_blank()) +
...
Meta-analysis analysis and figure: DELETE THIS!!!
```{r}
...
mean(intact.PM$gsw)
sd(intact.PM$gsw)
mean(intact.PM$calc.wp)
sd(intact.PM$calc.wp)
...
mean(intact.QG$gsw)
sd(intact.QG$gsw)
mean(intact.QG$calc.wp)
sd(intact.QG$calc.wp)
...
#Exclude Verryckt et al. 2020 - they only did one measurement per tree and only had 7 trees - also do not report means and sd, just median and range. Also exclude Lakso 1982 - no error terms and they don't state their n... Very old...

#Load Data:
RRDat <- read.xlsx("logRR_ExcisionData.xlsx")#%>%
  #filter(Reference != "Verryckt et al. 2020")%>%
  #filter(Reference != "Lakso 1982")
...
newRRDat$text_style <- ifelse(newRRDat$Species %in% bold_species, "bold.italic", "italic")

filtnewRRDat<-newRRDat%>%
  filter(lnRRr != "NA")
(t_test_lnRRs <- t.test(filtnewRRDat$lnRRr, filtnewRRDat$lnRR, paired = TRUE))
gfiltnewRRDat<-newRRDat%>%
  filter(glnRRr != "NA")
(t_test_glnRRs <- t.test(gfiltnewRRDat$glnRRr, gfiltnewRRDat$glnRR, paired = TRUE))

# Define the species to be bold
bold_species <- c("Acer campestre", "Betula papyrifera", "Carpinus betulus", "Pseudotsuga menziesii", "Quercus garryana")

# Add a new column to specify text style


# Plot for A that shows error bars
(lnRR_A = ggplot(newRRDat, aes(x = lnRR, y = Species, color = SignificantA)) +
  geom_point(shape = 18, size = 3) +  # shape 18 is a diamond
  geom_errorbarh(aes(xmin = lnRR - SE_lnRR, xmax = lnRR + SE_lnRR), height = 0.2, size = 1, na.rm = TRUE) +
  geom_vline(xintercept = 0, linetype = "dotted", color = "black") +
  labs(x = "Photosynthetic log response ratio", y = "") +
  theme_minimal() +
  theme(
    panel.grid = element_blank(),  # Remove all gridlines
    axis.line = element_line(color = "black"),  # Add x and y axis lines
    axis.text.y = element_text(face = newRRDat$text_style),  # Apply text style
    panel.border = element_rect(color = "black", fill = NA)  # Add border around the plot
  ) +
  scale_y_discrete(limits = rev(levels(newRRDat$Species))) +
  scale_color_manual(
    values = c("Yes" = "darkred", "No" = "lightblue"),
    labels = c("Yes" = "P < 0.05", "No" = "P > 0.05"),
    name="Significance"
  ))

# Plot for gsw that shows error bars
lnRR_gsw = ggplot(newRRDat, aes(x = glnRR, y = Species, color = Significantg)) +
  geom_point(shape = 18, size = 3) +  # shape 18 is a diamond
  geom_errorbarh(aes(xmin = glnRR - gSE_lnRR, xmax = glnRR + gSE_lnRR), height = 0.2, size = 1, na.rm = TRUE) +
  geom_vline(xintercept = 0, linetype = "dotted", color = "black") +
  labs(x = "Stomatal conductance log response ratio", y = "") +
  theme_minimal() +
  theme(
    panel.grid = element_blank(),  # Remove all gridlines
    axis.line = element_line(color = "black"),  # Add x and y axis lines
...
lnRR_plots = ggarrange(lnRR_A, lnRR_gsw, nrow=2, ncol=1, labels=c("A","B"), common.legend = TRUE,legend = "right")
# Assuming combined_plot is your plot object
ggsave("lnRR_plots.png", plot = lnRR_plots, width = 7, height = 9, dpi = 300)


# Create the barplot
plot_data <- newRRDat %>%
  select(Species, Xylem,lnRR, glnRR, text_style) %>%
  pivot_longer(cols = c(lnRR, glnRR), names_to = "Response", values_to = "Value")%>%
  group_by(Species, Xylem)


(g1 <- plot_data%>%
    group_by(Species, Xylem)%>%
  ggplot(aes(x = Value, y = interaction(Species, Xylem), fill = Response)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  theme_classic() +
  theme(plot.margin = unit(c(1, 1, 4, 1), "lines"))+
    scale_fill_manual(
    values = c("lnRR" = "darkred", "glnRR" = "lightblue"),
    labels = c("lnRR" = "Photosynthesis", "glnRR" = "Stomatal conductance"),
    name = " "
  ) +
  ...
  theme(
    panel.grid = element_blank(),  # Remove all gridlines
    axis.line = element_line(color = "black"),  # Add x and y axis lines
    axis.text.y = element_text(face = newRRDat$text_style, hjust = 1),
    panel.border = element_rect(color = "black", fill = NA)  # Add border around the plot
  )
)
...
create_subpanel <- function(data, column, show_x_title = TRUE) {
  ...
  print(paste("Filtered data for", column))
  print(filtered_data)
  ...
  plot <- filtered_data %>%
    ...
    labs(x = ifelse(show_x_title, "Log Response Ratio", ""), y = gsub("_", " ", column)) +
    theme_classic() +
    theme(
      panel.border = element_rect(color = "black", fill = NA),
...
exudates_panel <- create_subpanel(newRRDat, "Exudates", show_x_title = FALSE)
clade_panel <- create_subpanel(newRRDat, "Clade", show_x_title = FALSE)
drought_strategy_panel <- create_subpanel(newRRDat, "Drought_Strategy", show_x_title = TRUE)

# Combine all plots into one multi-panel plot using patchwork
multi_panel_plot <- (main_panel | (xylem_panel / exudates_panel / clade_panel / drought_strategy_panel)) + 
...
(percentDecr_BC.A = ((mean(Ex.BC$A) - mean(intact.BC$A)) / mean(intact.BC$A))*100)
...
(percentDecr_BC.g = ((mean(Ex.BC$gsw) - mean(intact.BC$gsw)) / mean(intact.BC$gsw))*100)
