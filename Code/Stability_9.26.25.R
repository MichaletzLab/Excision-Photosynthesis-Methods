#Filter based on auto-licor criteria
summary_branches <- full.exin.data %>%
  group_by(species, individual, branch) %>%
  summarise(
    co2h2o_ok = if (all(c("ΔCO2:OK", "ΔH2O:OK") %in% names(cur_data()))) {
      all(c_across(all_of(c("ΔCO2:OK", "ΔH2O:OK"))) == 1, na.rm = FALSE)
    } else {
      FALSE
    },
...
full.exin.data <- full.exin.data %>%
  filter(
    # Case 1: both ΔCO2 and ΔH2O exist and are 1
    (("ΔCO2:OK" %in% names(.)) & ("ΔH2O:OK" %in% names(.)) &
       `ΔCO2:OK` == 1 & `ΔH2O:OK` == 1) |
